<div class="modal-content">
        <div class="modal-header">
            <h3 class="modal-title">Edit Consumption</h3>
        ...
        <form id="updateForm" enctype="multipart/form-data">
            <div class="modal-body">
                ...
                <div class="fv-row">
                    <input type="text" name="name" id="name" class="form-control" placeholder="e.g. Nasi Goreng" value="<?= $consumption->name; ?>">
                </div>

                <label for="type" class="form-label fw-semibold required mt-2">Type</label>
                <div class="fv-row">
                    <select class="form-select" name="type" id="type" data-hide-search="true" data-control="select2" data-placeholder="Select an option">
                        <option></option>
                ...
                <label for="energy" class="form-label fw-bold mt-2 required">Energy</label>
                <div class="fv-row">
                    <input type="number" name="energy" id="energy" class="form-control" min="1" placeholder="e.g. 100" value="<?= $consumption->energy; ?>">
                </div>
            </div>
                ...
                form,
                {
                    fields: {
                        ...
                        'energy': {
                            validators: {
                                notEmpty: {
                                    message: 'Energy value is required'
                                },
                            }
                        },
                    },
                    ...
                    validator.validate().then(function (status) {
                        if (status == 'Valid') {
                            ...
                            setTimeout(function () {
                                ...
                                $.ajax({
                                    method: 'POST',
                                    url: base_url + "consumption/update",
                                    dataType: 'json',
                                    data: form,
                                    processData: false,
                                    contentType: false,
                                    success: function (result) {
                                        if (result.response == 200) {
                                            ...
                                            window.setTimeout(function () {
                                                window.location.href = base_url + 'consumption';
                                            }, 2000);
                                        } else {
                                            toastr.options = {
                                                "closeButton": false,
